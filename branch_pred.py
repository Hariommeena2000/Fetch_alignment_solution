...
def find_branch_conditions(parse_tree_lines):
    ...
    for i, line in enumerate(parse_tree_lines):
        if 'call' in line or 'if' in line or 'else' in line or 'while' in line:
            # The target of the branch is assumed to be the line following the branch instruction
            target = parse_tree_lines[i + 1].strip() if (i + 1) < len(parse_tree_lines) else 'Unknown'
            branch_conditions.append((line.strip(), target))
